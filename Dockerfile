...
RUN apt update && apt upgrade -y && apt-get install -y && apt-get install -y bc iputils-ping file ftp psmisc wget curl vim python3-pip lsb-release apt-transport-https gh ros-dev-tools ros-$ROS_DISTRO-rqt-tf-tree ros-$ROS_DISTRO-plotjuggler-ros && rm -rf /var/lib/apt/lists/*
